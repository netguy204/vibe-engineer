...
# Chunk: docs/chunks/task_aware_investigations - Task-aware investigation create
"""
...
class TestInvestigationCreateSelectiveProjects:
    ...
    def test_creates_external_yaml_only_in_specified_projects(self, tmp_path):
        ...
        result = runner.invoke(
            cli,
            [
                "investigation", "create", "memory_leak",
        ...
        assert (project_paths[0] / "docs" / "investigations" / "memory_leak" / "external.yaml").exists()
        assert (project_paths[2] / "docs" / "investigations" / "memory_leak" / "external.yaml").exists()

        # proj2 should NOT have external.yaml
        assert not (project_paths[1] / "docs" / "investigations" / "memory_leak").exists()
    ...
    def test_creates_external_yaml_in_all_projects_when_flag_omitted(self, tmp_path):
        ...
        result = runner.invoke(
            cli,
            ["investigation", "create", "memory_leak", "--project-dir", str(task_dir)],
        )
        ...
        for project_path in project_paths:
            assert (project_path / "docs" / "investigations" / "memory_leak" / "external.yaml").exists()
