...
Start a single `ve board watch-multi` command with all discovered changelog
channels. This uses one WebSocket connection for all channels instead of
N separate connections:

```
ve board watch-multi <channel1> <channel2> ... --swarm <swarm_id> [--server <url>]
```

Use `run_in_background` so the agent can continue interacting while the
watch runs. The `watch-multi` command auto-advances cursors after each
message is delivered and outputs messages tagged with their channel name.

Report to the operator which channels are being watched.

### Phase 4: Report incoming messages

Periodically check the background `watch-multi` task output. Each line
is formatted as `[channel-name] message text`.

For each new message line:

1. **Display the message** to the operator inline. Example:
...
2. Cursors are **auto-advanced** by `watch-multi` — no manual `ve board ack`
   is needed.

The monitoring continues until the operator stops the session or the
background task exits. If the background task exits unexpectedly, re-launch
it with `run_in_background`.

### Key Concepts

- **`watch-multi`** watches all channels on a **single connection**,
  reducing resource usage compared to N separate watch commands.
- **`run_in_background`** is how the agent waits asynchronously for messages.
  Do NOT poll. The `watch-multi` command blocks and streams messages.
