...
The cursor file is at `.ve/cursors/<channel>.cursor` (project-local). Read it
to get the current position N. If the file doesn't exist, the cursor is at
position 0. The next received message will be at position N+1 — you'll need
this for the ack step.

Example:
```
# Read current cursor position (may not exist yet — that's OK, treat as 0)
cat .ve/cursors/<channel>.cursor 2>/dev/null || echo 0
...
- Create a chunk or investigation as appropriate (`/chunk-create`,
  `/investigation-create`)
- Implement the work (create plan, implement, complete)
- Summarize the outcome for the changelog

**If mode is `queue`:**
- Analyze the inbound message to understand the request
- Create a chunk or investigation documenting the work item
- Do NOT implement — leave it for the human operator
...
### Step 6: Re-read SOP and rewatch
