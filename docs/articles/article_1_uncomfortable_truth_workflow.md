...
If every new session is like a new junior to onboard, then the bootstrap context (CLAUDE.md in my setup) is what HR handed them on the way in. But HR doesn't hand the junior the entire company wiki on day one—they'd drown. Agents have the same problem with a different name: the context window. The answer isn't a bigger wiki or a longer bootstrap context. It's documentation that can be discovered from wherever you start—code that links to the intent behind it, explorable as needed. And here's the leverage: every clarification you make to that documentation means every future agent understands your refined vision. Judgment applied to onboarding docs compounds in a way judgment applied to code never did.
