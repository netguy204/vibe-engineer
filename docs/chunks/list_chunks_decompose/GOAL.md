...
Decompose the 206-line `list_chunks` function in `src/cli/chunk.py` (lines 291-497) into focused, single-responsibility helpers. The function currently handles mutual exclusivity validation, multiple output mode branches, and full-list rendering with status filtering, external artifact handling, and JSON serialization all inline. This makes it difficult to read, test, and modify without risk of regressions.

This chunk extracts three pieces:

1. **Mutual exclusivity validation** (lines 314-344) -- Extract the four-way mutual exclusivity check between `--current`/`--last-active`/`--recent` and `--status`/`--future`/`--active`/`--implementing` into a Click callback or standalone validation helper, so the main function body starts at business logic rather than argument validation.

2. **Full-list rendering** (lines 402-497) -- Extract the `else` branch that handles iterating all chunks, checking external artifacts, parsing frontmatter, applying status filters, and emitting JSON or text output into its own function (`_render_full_chunk_list` or similar).

3. **Duplicated `status_matches` closure** in `src/cli/formatters.py` -- The identical closure appears at lines 81-92 and lines 163-175 of `formatters.py`. Extract it into a single module-level function that both `format_grouped_artifact_list_text` and `format_grouped_artifact_list_json` call.

Reducing function size and eliminating duplication keeps the codebase maintainable as the CLI grows, directly supporting the trunk goal that the workflow should not grow more difficult over time.

## Success Criteria

- The `list_chunks` function in `src/cli/chunk.py` is no longer responsible for mutual exclusivity validation inline; that logic lives in a dedicated callback or helper function.
- The full-list rendering branch (iterating chunks, handling external artifacts, parsing frontmatter, filtering by status, emitting JSON/text) is extracted into its own named function.
- The `list_chunks` function body is reduced to roughly 60 lines or fewer (excluding the extracted helpers), serving as a dispatcher that delegates to the appropriate branch.
- The `status_matches` closure is defined exactly once as a module-level function in `src/cli/formatters.py` and called from both `format_grouped_artifact_list_text` and `format_grouped_artifact_list_json`.
- No duplicate definition of `status_matches` remains in `formatters.py`.
- All existing tests pass (`uv run pytest tests/`) with no regressions -- the refactor is purely structural with no behavioral change.
- The CLI produces identical output for all `ve chunk list` invocations (plain list, `--current`, `--recent`, `--last-active`, `--status`, `--json`, and combinations) before and after the change.
