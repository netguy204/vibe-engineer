...
A marker file `docs/cursor_smoketest.md` exists at the repository root's `docs/`
directory, containing a single short line confirming that the orchestrator's
Cursor (Composer) backend executed a chunk end-to-end. This is a throwaway
validation chunk for the `backend_live_validation` effort: its only purpose is
to exercise the full orchestrator lifecycle (PLAN → IMPLEMENT → REVIEW →
COMPLETE) on the Cursor backend against a trivial, safe task.

## Success Criteria

- `docs/cursor_smoketest.md` exists and contains a single line of text
  confirming the Cursor backend ran (any clear one-line message is acceptable).
- No source code or other files are modified — this chunk only adds the one
  marker file.
...
This chunk is deliberately minimal. Its value is proving the orchestrator →
Cursor backend → Composer integration works end-to-end, not the change itself.
Anything more than a one-line marker file adds risk (Composer has no maxTurns
bound) without adding validation value.