...
Today, `src/templates/trunk/` ships eight `.jinja2` files (ARTIFACTS,
DECISIONS, EXTERNAL, FRICTION, GOAL, ORCHESTRATOR, SPEC, TESTING_PHILOSOPHY)
and `_init_trunk` in `src/project.py` renders the entire directory. There is
no `CHUNKS.md.jinja2` in that set, so a fresh `ve init` produces a project
without `docs/trunk/CHUNKS.md`. The canonical content exists at
`vibe-engineer/docs/trunk/CHUNKS.md` (24 lines describing the four
principles) but is not registered as a template that propagates.

This breaks the `/audit-intent` skill, whose first prerequisite check is
`test -f docs/trunk/CHUNKS.md || echo "MISSING: docs/trunk/CHUNKS.md — run
've init' to install."` The hint promises something `ve init` does not
deliver, so a new project that follows the documented onboarding fails the
audit-intent prerequisite. Operators currently work around this by hand-
copying `CHUNKS.md` from vibe-engineer's own checkout — friction that
breaks the "ve init gives you a working VE project" contract.

After this chunk, `ve init` on a fresh directory creates
`docs/trunk/CHUNKS.md` alongside the other trunk docs, and the audit-intent
skill's prerequisite hint becomes truthful.
