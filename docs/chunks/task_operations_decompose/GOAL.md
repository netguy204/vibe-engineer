...
- The four artifact-type-specific function families (`create_task_chunk`/`narrative`/`investigation`/`subsystem`, `list_task_chunks`/`narratives`/`investigations`/`subsystems`, `add_dependents_to_chunk`/`narrative`/`investigation`/`subsystem`) are replaced by generic implementations that accept an artifact type parameter
