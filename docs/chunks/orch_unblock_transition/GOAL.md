...
- `ve orch ps` output shows no stale reasons for active work units
- Test coverage for the unblock-to-ready transition path
- Test coverage for reason cleanup on status transitions
...
**Related issue:** The `attention_reason` field also persists when work units are
...
should clear `attention_reason` on any transition to READY or RUNNING, not just
the automatic unblock case.