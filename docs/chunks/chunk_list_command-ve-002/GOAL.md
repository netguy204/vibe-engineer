...
- ref: src/cli/chunk.py#list_chunks
  implements: CLI command ve chunk list with --current, --last-active, --recent, and status filtering options
- ref: tests/test_chunks.py
  implements: Unit tests for list_chunks() and get_latest_chunk() methods
- ref: tests/test_chunk_list.py
  implements: CLI integration tests for ve chunk list with status filtering and external artifacts
created_after:
