...
resume_session_id, answer, plus orchestrator policy callbacks), `ToolUse` and
`ToolDecision` for tool interception, and the existing
`AgentResult` (`src/orchestrator/models.py`, reused unchanged). Orchestrator
policy — worktree sandbox enforcement, operator question/suspend forwarding, and
review-decision capture — is expressed as callbacks on `SessionRequest`
(`on_tool_use` → ALLOW/DENY, `on_question`, `on_review_decision`, plus
`on_log`), which `ClaudeBackend` maps onto the existing SDK hooks. The
abstraction owns the seam at which a second backend (e.g. Cursor/Composer) plugs
in without touching the scheduler, work-unit state machine, or worktree
management.

## Success Criteria

- An `AgentBackend` Protocol (or ABC) exists with a single entry point —
  e.g. `async run(request: SessionRequest) -> AgentResult` — covering both fresh
  runs and resume (resume folded in via `resume_session_id`/`answer` on the
  request).
