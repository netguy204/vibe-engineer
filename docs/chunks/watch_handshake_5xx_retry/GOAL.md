...
The handshake-transient-as-retryable semantic established by
`watch_handshake_timeout_retry` and unified across both reconnect branches by
`watch_handshake_stale_retry` extends to **server-side 5xx responses during
the WebSocket opening handshake**. A `websockets.exceptions.InvalidStatus`
raised with an HTTP 5xx status code is treated identically to
`TimeoutError` / `ConnectionClosedError` — it routes through the shared
...
reconnect handshake no longer kills a long-lived watch.
...
boundary" — covers this case in spirit but the implementation does not.
This chunk closes the remaining gap: `InvalidStatus` with HTTP 5xx is
the same class of transient blip that `TimeoutError` represents and
joins the retryable set in `_connect_with_retry`. The 4xx case stays
explicitly outside the retryable set so genuine configuration errors
remain visible.