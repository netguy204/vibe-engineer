...
Diagnose and fix persistent WebSocket disconnects on the Cloudflare DO worker. Despite client-side `ping_interval=20` and `ping_timeout=10`, connections still drop every 5-15 minutes across all channels. The backoff reset fix (`websocket_reconnect_tuning`) confirmed the disconnects are real — reconnects consistently happen at ~1s intervals.

Field data (post-fix):
- vibe-engineer-changelog: 6 disconnects over 70 min (intervals: 25m, 16m, 6m, 13m, 11m)
- repro-maa-changelog: 7 disconnects over 55 min (intervals: 1m, 8m, 14m, 11m, 13m, 8m)
- database-savings-plans-changelog: 4 disconnects over 15 min

Hypotheses to test:
1. **Hibernation breaks WebSocket pong**: The docs say "Ping/pong handling does not interrupt hibernation" but maybe the DO's WebSocket handler needs to use `state.acceptWebSocket()` with specific tags or the hibernation handler needs `webSocketClose`/`webSocketError` handlers to properly maintain connections
2. **Cloudflare proxy idle timeout**: There may be a Cloudflare edge/proxy timeout that's independent of the DO — the proxy terminates idle connections regardless of ping/pong at the DO level
3. **Client ping_timeout too aggressive**: With `ping_timeout=10`, if a single pong is delayed (e.g., DO waking from hibernation takes >10s), the client closes the connection. Try increasing `ping_timeout` to 30s or disabling it
4. **Missing `webSocketClose` handler**: The DO may not be properly handling WebSocket lifecycle events during hibernation, causing Cloudflare to clean up connections

Diagnostic approach: add server-side logging (via `console.log` visible in `wrangler tail`) to track when WebSockets are accepted, when pings arrive, and when connections close — then correlate with client disconnect timestamps.

## Success Criteria

- Root cause identified and documented
- Fix implemented that reduces disconnect frequency to <1 per hour on idle channels
- Client-side reconnect still works as a safety net
- DO hibernation still functions (no regression to alarm-based heartbeats)

## Relationship to Parent

Parent chunk `websocket_reconnect_tuning` fixed backoff reset and investigated the issue. This chunk does the deeper diagnostic work — instrumenting both client and server to find the actual cause of the drops.