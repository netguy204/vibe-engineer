...
flatly contradicts the `/narrative-create` skill prompt
(`src/templates/commands/narrative-create.md.jinja2`) on whether the
`proposed_chunks` array should be populated at narrative-creation time.

- The skill prompt's Step 4 walks the agent through populating
  `proposed_chunks` and explains the `depends_on` semantics.
- The OVERVIEW.md template's inline frontmatter comment instructs
  "DO NOT POPULATE this array during narrative creation. It will be populated
  as chunks are created."

The operator's stated intent is that `proposed_chunks` MUST be populated at
narrative-creation time. Each entry enumerates an unreified chunk in the
PROPOSED state, which is the affordance that lets `ve chunk list` (and
`ve chunk list-proposed`) surface work the operator has spec'd but not yet
`/chunk-create`d. Leaving the array empty defeats that surface.

A real agent (operator-reported) followed the emphatic "DO NOT POPULATE"
inline note over the skill's Step 4, submitted an empty `proposed_chunks: []`,
and the narrative was unusable as a discovery surface for downstream work.
This bug recurs every time the contradiction is encountered.

This chunk reconciles the two sources of guidance so that the authoritative
behavior — populate at narrative-creation time — is reflected consistently.
