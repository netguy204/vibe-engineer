...
selection is surfaced through `ve settings` / orchestrator configuration
alongside the existing turn-budget settings (`max_turns_implement`,
`max_turns_complete`), so an operator opts into a non-Claude backend with one
config value and no code change.
...
- The setting is readable/writable through the same surface as other orchestrator
  config (`ve settings` and/or the `OrchestratorConfig` load path).
