...
This chunk fixes a critical correctness bug in the orchestrator's SQLite state store. In `src/orchestrator/state.py`, the `get_ready_queue` and `get_attention_queue` methods use `LIKE '%"chunk_name"%'` to search the JSON array stored in the `blocked_by` column. This creates false-positive matches when one chunk name is a substring of another (e.g., chunk "foo" would incorrectly match "foobar" in the JSON).

The correct pattern already exists in the same file: `list_blocked_by_chunk` uses SQLite's `json_each()` function for reliable JSON array element matching. This chunk replaces all LIKE-based blocked_by queries with `json_each()` for consistency and correctness.

## Success Criteria

- All blocked_by queries in `state.py` use `json_each()` instead of LIKE-based JSON matching
- No LIKE patterns remain for JSON array searching in the state store
- A test demonstrates that substring chunk names (e.g., "foo" vs "foobar") are correctly distinguished
