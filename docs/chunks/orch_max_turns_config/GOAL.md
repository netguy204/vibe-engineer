...
configuration rather than hardcoded literals in `orchestrator/agent.py`.
Operators set the IMPLEMENT-phase budget (and the COMPLETE-phase budget,
which today is a separate hardcoded value) via `ve orch config`, the same
surface that already owns `max_agents`, `dispatch_interval_seconds`, and
`worktree_warning_threshold`. The agent invocation reads from that config at
dispatch time, with the existing values (100 / 20) as defaults so behavior
is preserved when no override is set.

## Success Criteria

- `ve orch config` accepts `--max-turns-implement` and
  `--max-turns-complete` (or equivalent) and persists them through the same
  mechanism as the existing knobs.
- `ve orch config` (no args) prints the current values alongside the others.
- `agent.py` reads the budgets from config; the literals `max_turns=100`
  (line 623) and `max_turns=20` (line 872) are gone.
- Defaults match today's values (100 implement, 20 complete) — unchanged
  behavior when operator hasn't set anything.
- Daemon picks up changes on restart (matches behavior of existing config
  fields).
- Operators no longer have to edit installed package files to raise the
  budget for large refactor chunks.