...
subsystems: []
friction_entries: []
depends_on: ["plugin_init_slimdown"]
...
- Migration must be idempotent: re-running on an already-migrated project is
  a no-op, reported via the existing InitResult created/skipped/warnings
  channels.
