...
Fix bug where newly created invite tokens are immediately invalid. `ve board invite create` succeeds (HTTP 200 from PUT to `/gateway/keys`) and returns an invite URL, but curling the URL immediately returns "Invalid or expired invite token".

Likely causes to investigate:
- **Token hash mismatch**: The Python CLI (`src/board/crypto.py`) and the TypeScript server (`workers/leader-board/src/gateway-crypto.ts`) may compute `hash(token)` differently (e.g., different encoding of the token before hashing, hex vs raw bytes, different SHA-256 input formats)
- **Encrypted blob format mismatch**: The CLI encrypts with one NaCl secretbox format but the server expects a different layout (nonce prefix, key derivation differences)
- **KV index not populated**: The `invite_path_routing_fix` added a KV token→swarm index; if the PUT route doesn't write to KV, the invite URL routing fails before reaching the DO
- **Swarm ID mismatch in storage**: The blob is stored in one DO but the invite lookup routes to a different DO

The fix should include an end-to-end test: create token via CLI → curl invite URL → verify instruction page is returned.
...
Parent chunk `invite_cli_command` implemented the create flow. This bug indicates a mismatch between the CLI's token/blob format and the server's expectations, or a routing issue introduced by `invite_path_routing_fix`.