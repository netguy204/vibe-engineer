...
- Remove: `_init_skills()` and the .claude/commands symlink machinery in
  src/project.py (the skills phase of `Project.init()`), plus the
  src/templates/commands/ collection and its partials. Note
  `_is_ve_generated_file()` (src/project.py) is still needed by
  plugin_legacy_migration's cleanup — keep it.
- Shrink: the managed content in src/templates/claude/AGENTS.md.jinja2 (and
  CLAUDE.md.jinja2), which today documents every command and workflow. Reduce
  to trunk-doc pointers, chunk conventions, and a line directing agents/users
  to the plugin for commands.
- Keep: `_init_trunk()`, `_init_narratives()`, `_init_chunks()`,
  `_init_reviewers()`, `_init_gitignore()`, and the magic-marker machinery
  (parse_markers and friends in src/project.py).
- `ve task init` (src/task_init.py) also renders skills into task roots —
  slim it equivalently; it still writes .ve-task.yaml and the task-level
  AGENTS.md.
- Update or remove tests that cover skill rendering and symlink creation.
- This chunk depends on both command-port chunks: removing the render channel
  before the plugin carries all 36 commands would strand users.
