...
After this chunk owns its intent, an entity that has just finished
implementing a chunk and discovered that scope landed entirely inside
one repo can collapse the cross-repo bookkeeping in one command,
removing the friction that today produces 17+ stale pointer dirs and
misleading `dependents:` frontmatter.
