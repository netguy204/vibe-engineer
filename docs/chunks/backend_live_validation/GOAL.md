...
The orchestrator's Cursor backend is validated against Composer in a live,
end-to-end run — not just mock-based unit tests. With `cursor-agent` installed
and authenticated to a Composer-capable account and `backend=cursor` selected
(`ve orch config --backend cursor`), a representative chunk completes the full
lifecycle (PLAN → IMPLEMENT → REVIEW → COMPLETE) through the orchestrator on
Composer. The live run confirms the in-worktree behaviors the unit tests can
only mock: sandbox enforcement denies a worktree-escaping command via ACP
`session/request_permission`, an operator question suspends and forwards via
`cursor/ask_question`, and the `ReviewDecision` MCP tool records a decision.
Divergences observed live (e.g. the documented ACP `maxTurns` gap) are
reconciled against the `docs/trunk/ORCHESTRATOR.md` divergence table, and any
prompt or per-phase turn-budget tuning is applied.

This chunk completes the operator-gated portion of `backend_parity`
(`docs/chunks/backend_parity`), which delivered the divergence analysis,
documentation, bug fix, and tests but could not perform live Composer runs in an
autonomous worktree (no `cursor-agent` binary / Composer subscription). It is
deliberately operator-run.

## Success Criteria

- `cursor-agent` is installed and authenticated; `ve orch config --backend
  cursor` selects the Cursor backend.
- At least one representative chunk completes the full lifecycle
  (PLAN → IMPLEMENT → REVIEW → COMPLETE) on the Cursor backend through the
  orchestrator — a real Composer run, not a mock.
- In that run, observed live: sandbox enforcement blocks a worktree-escaping
  command; an operator question suspends the run and is answerable; a REVIEW
  produces a captured `ReviewDecision`.
- Live-run divergences from Claude are recorded/reconciled in
  `docs/trunk/ORCHESTRATOR.md`; any prompt or `max_turns_*` tuning is applied.
- Any blocking ACP/Composer limitation found live (e.g. no `maxTurns`) is
  documented with the chosen mitigation.

## Rejected Ideas

### Forcing live runs inside an autonomous orchestrator worktree

`backend_parity` already showed this fails: a worktree agent has no
`cursor-agent` binary or Composer subscription, so it can only mock. Live
validation is deliberately operator-run rather than another autonomous chunk.