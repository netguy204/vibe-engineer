...
1. The scheduler performs `git merge main` into the worktree branch (mechanical step, no agent needed)
2. If the merge is clean — advance directly to REVIEW with no agent invocation
3. If the merge has conflicts — spawn an agent that:
   - Reads the chunk's GOAL.md to understand what this chunk is trying to accomplish
   - Resolves each conflict in light of that intent (keep the chunk's changes where they implement the goal, accept trunk changes elsewhere)
   - Runs the project's test suite to verify the resolution
   - Commits the merge
4. If the agent cannot resolve conflicts — mark NEEDS_ATTENTION for operator help
...
- On entering REBASE, the scheduler first attempts a mechanical `git merge main` in the worktree
- If the merge is clean (no conflicts), the phase advances to REVIEW immediately without spawning an agent — this is the fast path and should add negligible latency for non-conflicting branches
- If the merge has conflicts, an agent is spawned with the chunk's GOAL.md as context and tasked with resolving conflicts, then running tests
- If the agent resolves conflicts successfully, the merge is committed and the phase advances to REVIEW
- If the agent fails to resolve or tests fail, the work unit is marked NEEDS_ATTENTION with a descriptive reason including which files conflicted
...
- All existing scheduler tests pass; new tests cover: clean merge (fast path), conflicting merge (agent resolution), and unresolvable merge (NEEDS_ATTENTION)
...
### Mechanical-only merge with no agent

We considered only doing the merge mechanically and immediately marking NEEDS_ATTENTION on any conflict. Rejected because: the operator explicitly requested that conflicts be resolved in light of the chunk's intent before the reviewer sees them. Most merge conflicts in this codebase are trivial (status field changes, backreference comments) and an agent can resolve them with high confidence.