...
The chunk-create command is ported end-to-end as the pilot demonstrating the
convention.

## Context

- Render-time machinery being replaced: src/template_system.py —
...
- `ve task init` writes `.ve-task.yaml` into the task root — that file is the
  ...
  project root.
- The convention must be mechanically applicable: chunks
  plugin_core_commands and plugin_orch_commands mass-apply it to the remaining
  35 commands, so write it as a reusable preamble/shared snippet plus a short
  porting guide.
- The auto-generated header ("AUTO-GENERATED FILE - DO NOT EDIT DIRECTLY /
  ...
  the source, not render output. Drop it from ported commands.
