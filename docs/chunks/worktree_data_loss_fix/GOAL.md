...
This chunk fixes a critical data loss bug in the orchestrator's worktree manager. In `src/orchestrator/worktree.py`, the `_update_working_tree_if_on_branch` method (around line 1152-1191) runs `git checkout -- .` after `git reset --mixed HEAD` when the user is on the target branch after a ref update. The `git checkout -- .` command silently overwrites uncommitted changes in the user's working directory, meaning if the user has uncommitted work in files also modified by the merged branch, those changes are permanently lost.

The fix replaces the destructive `git checkout -- .` with a safe alternative (such as `git read-tree -m -u HEAD`) that will fail with an error if there are conflicts with uncommitted changes rather than silently overwriting them. This is the right next step because data loss bugs undermine trust in the tool.

## Success Criteria

- `_update_working_tree_if_on_branch` no longer runs `git checkout -- .` or any other command that silently discards uncommitted changes
- When the user has uncommitted changes that conflict with merged changes, the operation fails with a clear error message instead of silently overwriting
- When the user has no uncommitted changes, the working tree is still updated correctly
- Existing tests pass; new test covers the conflict-with-uncommitted-changes scenario
