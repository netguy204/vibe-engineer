...
After this chunk owns its intent, the message "Resolve unresolvable
conflicts manually and commit" is matched by an entity worktree that
actually contains those conflicts.
...
- An explicit `ve entity merge --abort` command (or equivalent) exists
  for the operator to opt into discarding the in-progress merge — the
  capability that today's silent abort took away.
