...
- Hook configuration lives in the plugin's hooks/hooks.json.
- Version sources: the Python package version (pyproject.toml, `ve --version`)
  vs the version in .claude-plugin/plugin.json. The compatibility policy is
  defined in this chunk (e.g., matching minor versions, or plugin.json
  declaring a supported ve version range) and documented.
- "Is a ve project" detection: pick a cheap signal (e.g., docs/trunk/GOAL.md
  or docs/chunks/ present) and document the choice.
- Surface the current chunk via `ve chunk list --current`.
- Keep hook output to a few lines — it is prepended to every session in
  consuming projects.
