...
- The `ve` command is an installed CLI tool. Run via Bash.
...
# (2) Chunk GOAL template must declare COMPOSITE in its STATUS VALUES block
...
2. Spawn 10 sub-agents in parallel via the Agent tool, subagent_type `general-purpose`. **All 10 calls must be in a single message** so they run concurrently.
3. Each sub-agent receives the full audit protocol (see "Sub-agent prompt template" below) plus its specific 5-chunk scope.
