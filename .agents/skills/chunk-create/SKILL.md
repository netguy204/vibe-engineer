...
4. **Apply the intent-judgment gate before refining the goal.**

   <!-- This gate enforces docs/trunk/CHUNKS.md principle 2 — chunks exist only
        for intent-bearing work. The asymmetry is load-bearing: operators routinely
        spawn entire narratives' worth of chunks in one prompt. Re-confirming each
        would be unbearable. The agent only interrupts on suspected scope creep. -->
...
6. **Check if this chunk comes from a narrative or investigation with dependencies.**
...
7. **Understand the `depends_on` null vs empty semantics.**
...
8. **Check if this is a bug fix.** Look for these signals that indicate bug fix
...
9. **Check for existing implementing chunk.** Run `ve chunk list --current` to
