...
description: Send a request and wait for the response on a channel pair. Use when the operator wants a request-response exchange with another project's steward over swarm channels — send to the steward channel, watch the changelog for the reply.
...
**Derive channels from the TARGET project.** When the request is addressed to
another project's steward, both channel names come from the project whose
steward you're addressing — `<target-project>-steward` /
`<target-project>-changelog` — **not** the project you're sending from. For
example, to ask the `vibe-engineer` steward for something from any project in
the swarm, send to `vibe-engineer-steward` and watch
`vibe-engineer-changelog`.
...
the target project's channel. Always derive the channel name from the
...
1. Ack the response channel to advance past this message:
   ...
   ```
2. Re-launch the watch in background (the persisted cursor has advanced via
   ack, so no `--offset` is needed now):
   ```
   ve board watch <response_channel> --swarm <swarm_id> [--server <url>]
   ```
   Run with `run_in_background`. Record the new task ID.
3. Continue waiting for the next message.

**If the message IS relevant:**
1. Return the response content to the operator
...
- **Channel pair convention** — `<project>-steward` (request) /
  ...
  can form a request-response pair. Both names derive from the **target**
  project — never from the sender's local steward configuration.
