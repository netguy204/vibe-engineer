...
def ensure_cached(repo: str) -> Path:
    ...
    Uses bare clone for space efficiency since we only need to read content.
    ...
    if cache_path.exists():
        ...
        try:
            ...
            raise ValueError(
                f"Failed to fetch updates for '{repo}': {e.stderr.strip() if e.stderr else 'unknown error'}"
            ) from e
    else:
        # Clone as bare repo
